...
char* indexes(const char* path)
{
    // make two strings with new path names
    char* indexa = malloc(strlen(path) + 10), 
         *indexb = malloc(strlen(path) + 11);

    indexa = strcpy(indexa, path);
    indexa = strcat(indexa, "index.php");
    indexb = strcpy(indexb, path);
    indexb = strcat(indexb, "index.html");
    
    if (access(indexa, R_OK) != -1)
    {
        return indexa;
    }
    else if (access(indexb, R_OK) != -1)
    {
        return indexb;
    }
    else
...
const char* lookup(const char* path)
{
    ...
    // set constants for comparision
    const char* css = ".css", *html = ".html", *gif = ".gif", *ico = ".ico",
                *jpg = ".jpg", *js = ".js", *php = ".php", *png = ".png";
    
    // compare and return appropriate value
    if (strcasecmp(css, suffix) == 0)
        return "text/css";
    else if (strcasecmp(html, suffix) == 0)
        return "text/html";
    else if (strcasecmp(gif, suffix) == 0)
        return "image/gif";
    else if (strcasecmp(ico, suffix) == 0)
        return "image/x-icon";
    else if (strcasecmp(jpg, suffix) == 0)
        return "image/jpeg";
    else if (strcasecmp(js, suffix) == 0)
        return "text/javascript";
    else if (strcasecmp(php, suffix) == 0)
        return "text/x-php";
    else if (strcasecmp(png, suffix) == 0)
        return "image/png";
    else
        return NULL;
}
...
{
    // declare constants for searching
    const char* get = "GET ", *ver = "HTTP/1.1", *spaces = "  ",
                *quote = "\"", *crlf = "\r\n",
                *last2 = &line[strlen(line) - 2];
    
    const char qmark = '?', slash = '/', space = ' ';
    char* buffer = strchr(line, space);
    
    // separate parts of request into abs_path, and query
    for (int i = 1; buffer[i] != 32; i++)
    {
        abs_path[i - 1] = buffer[i];
    }
    
    buffer = strchr(abs_path, qmark);
    if (buffer != NULL)
    {
    ...
    // reject bad requests
    if (strncmp(get, line, strlen(get)) != 0)
    {
        error(405);
        return false;
    }
    else if(strstr(line, ver) == NULL)
    {
        error(505);
        return false;
    }
    else if(strncmp(abs_path, &slash, 1) != 0)
    {
        error(501);
        return false;
    }
    else if(strstr(line, spaces) != NULL || 
            strstr(abs_path, quote) != NULL ||
            strcmp(last2, crlf) != 0)
    {
        error(400);
        return false;
